...
export function AdjustStockModal({
  ...
  // Adjustments target the selected location's stock row in
  // product_stock_by_location -- that table is the source of truth for
  // stock tracking going forward, not seller_products.stock_quantity
  ...
  const nextQuantity = Math.max(0, (currentQuantity ?? 0) + changeAmount);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const { error: updateError } = await supabase
      ...
      .upsert(
        { product_id: product.id, location_id: locationId, stock_quantity: nextQuantity },
  ...
  return (
    <Modal title={`${t("Adjust Stock")} — ${product.product_name}`} onClose={onClose}>
      <form onSubmit={submit} className="flex flex-col gap-3">
        ...
        <div className="mt-2 flex justify-end gap-2">
          ...
          >
            {saving ? t("Saving…") : t("Apply adjustment")}
          </button>
        </div>
      </form>
    </Modal>
